...
type during decryption, some sort of header information has to be stored.

The following information is encoded into the first cryptographic block during
encryption:

\begin{itemize}
\item length of the salt
\item the salt
\item original data type of the value (string, binary, null)
\item original content length
\end{itemize}
...
of data. The block size depends on the cryptographic algorithm that is
used.
