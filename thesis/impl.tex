...
All calls that the \fcrypt ~produces can be executed in a shell without Elektra.
The calls rely on the \texttt{fork} and the \texttt{execv} system functions.

\subsubsection{File Based Encryption and Decryption}

The encryption is done in the \texttt{kdb set} method of the \fcrypt.
...
gpg2 --batch -o /tmp/config.asc --yes -r 7F230E8D --armor \
	-e /tmp/plaintext.ini
\end{lstlisting}
...
The verification of the signature works like a regular decryption and has already been shown in Listing \ref{list-fcrypt-dec} on page \pageref{list-fcrypt-dec}.
